...
╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝  ╚═════╝ ╚══════╝   ╚═╝   ╚══════╝╚═╝  ╚═╝
{Color.RESET}
{Color.GREEN}  Advanced Web Directory & File Bruteforcer  v1.2.0{Color.RESET}
...
def get_wordlist_path(name):
    ...
    if os.path.isabs(name):
        return name
    if name.startswith("wordlists/") or name.startswith("wordlists\\"):
            ...
            for root, _, files in os.walk(wl_dir):
                for file in sorted(files):
                    if file.endswith(".txt"):
                        rel = os.path.relpath(os.path.join(root, file), wl_dir)
                        print(f"  - {rel}")
        sys.exit(1)
...
# ─────────────────────────────────────────
#  SCANNER  — FIX: task_done() inside outer loop
# ─────────────────────────────────────────
def scanner(queue, base_url, extensions, timeout, status_codes, output_file, session):
    ...
    while not stop_event.is_set():
        ...
        queue.task_done()  # FIX: moved here — after processing all targets for a word

# ─────────────────────────────────────────
#  PROGRESS — FIX: added while loop
# ─────────────────────────────────────────
    ...
    for root, _, files in os.walk(base):
        for file in sorted(files):
            if file.endswith(".txt"):
                ...
                try:
                    count = sum(
                        1 for line in open(fpath, encoding="utf-8", errors="ignore")
                        if line.strip() and not line.startswith("#")
                    )
                except:
                    count = 0
                print(f"  {green(rel)} {yellow(f'({count} entries)')}")
    print()
...
def main():
    ...
    parser.add_argument("-w", "--wordlist",     help="Wordlist to use (default: common.txt)", default="common.txt")
    parser.add_argument("-t", "--threads",      help="Number of threads (default: 20)", type=int, default=20)
    parser.add_argument("-x", "--extensions",   help="File extensions to test (e.g. .php,.html,.bak)", default="")
    parser.add_argument("-o", "--output",       help="Save results to file")
    parser.add_argument("-s", "--status",       help="Status codes to show (default: 200,301,302,307,401,403)", default="200,301,302,307,401,403")
    parser.add_argument("--timeout",            help="Request timeout in seconds (default: 5)", type=int, default=5)
    parser.add_argument("--user-agent",         help="Custom User-Agent string")
    parser.add_argument("--cookie",             help="Cookie header value (e.g. 'session=abc123')")
    ...
    ua = args.user_agent or "DarkBuster/1.2 (github.com/darkdisaster08)"
    ...
    if args.output:
        with open(args.output, "w") as f:
            f.write(f"DarkBuster v1.2.0 Scan Results\n")
            f.write(f"Target  : {base_url}\n")
            f.write(f"Date    : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Wordlist: {os.path.basename(wl_path)}\n")
            f.write("=" * 60 + "\n\n")
    ...
    for _ in range(min(args.threads, len(words))):
        t = threading.Thread(
            target=scanner,
            args=(q, base_url, extensions, args.timeout, status_codes, args.output, session)
        )
        t.daemon = True
        t.start()
        threads.append(t)
